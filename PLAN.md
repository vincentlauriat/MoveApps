...
### Phase 4 — Packaging & multi-Mac distribution (in progress, started 2026-07-03)
`Scripts/release.sh` full pipeline (codesign, notarize, DMG) already existed from Phase 0 scaffolding and turned out largely complete. Done: `Info.plist` polish (`LSApplicationCategoryType`/`NSHumanReadableCopyright` via `project.yml`), confirmed empty entitlements are correct for this non-sandboxed Hardened Runtime app, added a `SKIP_NOTARIZE=1` local dry-run mode, ran and independently re-verified the full build→codesign→DMG chain (real Developer ID signature, Hardened Runtime on, `codesign`/`spctl` both pass). **Blocked** on notarization: the `MoveApps-Notary` keychain profile isn't set up yet — needs Vincent to run `xcrun notarytool store-credentials` once with a fresh Apple ID app-specific password (see `TODOS.md` for the exact command; not automatable). AppIcon left as a placeholder (non-blocking, cosmetic). v1: manual distribution (private repo, no Sparkle network feed yet). Still to validate: first-launch flow on a fresh Mac (`NSOpenPanel` root selection, bookmarks are per-Mac).
