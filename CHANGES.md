...
### Added (project index generation, 2026-07-04)
- **`IndexGenerator` (MoveAppsCore)** — new pure `Sendable` service that builds one unified `INDEX.md` covering **both** roots and writes an **identical copy into each** (`~/DevApps/INDEX.md` and `~/Documents/GitHub/INDEX.md`). Answers Vincent's request that the two-directory project catalog be generated by MoveApps itself and mirrored in both places. Reuses `ProjectScanner` (same project list as the transfer UI, container folders unpacked), groups by root → category folder → root-level, renders `**name** — \`path\` — stack — description` per entry.
- **README description extraction** — descriptions are pulled from each project's `README.md` (first meaningful prose line). Hardened filter (`firstMeaningfulLine`/`cleanMarkdown`/`isProse`/`startsWithCommand`) skips headings, badges, images, raw HTML and stray tag attributes (any `="`), ASCII-art banners (letter-ratio check), multilingual nav bars (`≥3` pipe-separated segments), label lines (trailing `:`) and shell/install commands (`curl`/`npm`/`irm`/…), decodes HTML entities (`&nbsp;`/`&amp;`/…), and falls back to the H1 title. Validated against the real roots — noise lines like claude-mem's `src="…"`, headroom's ASCII banner, oh-my-claudecode's language selector and feynman's install command are all correctly skipped in favor of real prose or the title.
- **Two triggers**: a "Régénérer l'index" button on the menu-bar dashboard (`DashboardViewModel.regenerateIndex`, inline success/failure banner) and automatic regeneration after every transfer/batch (`MainWindowViewModel.regenerateIndex`, fire-and-forget off the main actor). Per-project disk size deliberately omitted (a `du` per project is too slow for a regenerate-on-every-transfer action).

### Validation (project index)
- New `IndexGeneratorTests` (8 tests): writes identical copies to both roots covering projects from each, README prose extraction skipping headings/badges/HTML, ASCII-art/nav-bar/HTML-fragment skipping, HTML-entity decoding, H1 title fallback, word-boundary truncation. Full suite **39 tests / 12 suites, `TEST SUCCEEDED`**; app `BUILD SUCCEEDED` (Swift 6 strict concurrency, no warnings).
- End-to-end on the **real roots** (disposable scratch test calling `IndexGenerator().write(roots: .default)`, exactly as the app does): both `~/DevApps/INDEX.md` and `~/Documents/GitHub/INDEX.md` written and byte-identical, 104 projects (25 active + 79 archived) — the scratch test was removed afterward, the two real `INDEX.md` files are the deliverable.
